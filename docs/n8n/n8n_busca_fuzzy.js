/**
 * Código para n8n - Busca Fuzzy
 * 
 * Este código deve ser colocado no node "Code" do n8n
 * para processar inventário com busca fuzzy antes de chamar Claude.
 * 
 * IMPORTANTE: Este código é uma versão adaptada para n8n.
 * As funções são definidas dentro do código do n8n.
 */
...
const baseIngredientes = $('Code - Format Base Ingredientes').first().json.base_ingredientes;
const inventarioTexto = $input.first().json.inventario_extraido || '';

if (!baseIngredientes || !Array.isArray(baseIngredientes)) {
...
for (const ingredienteTexto of ingredientes) {
  ...
  resultado.nome_original = nomeIngrediente;
...
return {
  json: {
    ...
    total_processados_fuzzy: ingredientes_processados.length,
    total_para_claude: ingredientes_para_claude.length
  }
};
